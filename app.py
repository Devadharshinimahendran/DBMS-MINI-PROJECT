...
def rooms():
    ...
    cur.close()
    return render_template('homepage.html', rooms=room_info)  # Render room info
...
def search():
    ...
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        query = "SELECT * FROM rooms WHERE id LIKE %s"
...
def insert():
    if request.method == "POST":
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO rooms (id, room_type, price) VALUES (%s, %s, %s)", (room_id, room_type, price))
...
def delete(id_data):
    cur = mysql.connection.cursor()
    cur.execute("DELETE FROM rooms WHERE id=%s", (id_data,))
    mysql.connection.commit()
    return redirect(url_for('rooms'))
...
def edit(id_data):
    cur = mysql.connection.cursor()
    cur.execute("SELECT * FROM rooms WHERE id=%s", (id_data,))
...
def update():
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("UPDATE rooms SET room_type=%s, price=%s WHERE id=%s", (room_type, price, room_id))
        mysql.connection.commit()
